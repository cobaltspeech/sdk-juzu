...
func TestGateway(t *testing.T) {
	...
	err = gw.RegisterJuzuHandlerFromEndpoint(context.Background(), gwmux, fmt.Sprintf("localhost:%d", port), opts)
	if err != nil {
...
func (s *stubServer) Version(ctx context.Context, e *empty.Empty) (*juzupb.VersionResponse, error) {
	return nil, fmt.Errorf("not implemented")
}

func (s *stubServer) ListModels(ctx context.Context, r *empty.Empty) (*juzupb.ListModelsResponse, error) {
	return nil, fmt.Errorf("not implemented")
}

func (s *stubServer) StreamingDiarize(stream juzupb.Juzu_StreamingDiarizeServer) error {
...
func setupGRPCServer() (*grpc.Server, int, error) {
	...
	s := grpc.NewServer()
	juzupb.RegisterJuzuServer(s, &stubServer{})

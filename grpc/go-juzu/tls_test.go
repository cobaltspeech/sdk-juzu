...
import (
	...
	"github.com/cobaltspeech/sdk-juzu/grpc/go-juzu"
...
func TestServerTLS(t *testing.T) {
	...
	if _, err := juzu.NewClient(addr, juzu.WithConnectTimeout(200*time.Millisecond)); err == nil {
	...
	if _, err := juzu.NewClient(addr, juzu.WithServerCert(certPem),
	...
	if _, err = juzu.NewClient(addr, juzu.WithServerCert(certPem[:3])); err == nil {
		t.Errorf("client with invalid root-CA: want error, got nil")
	}

	// insecure connection should also fail
	if _, err := juzu.NewClient(addr, juzu.WithInsecure(),
		juzu.WithConnectTimeout(200*time.Millisecond)); err == nil {
...
-----END PRIVATE KEY-----`)
	...
	if _, err := juzu.NewClient(addr, juzu.WithClientCert(certPem, keyPem),
		juzu.WithServerCert(certPem), juzu.WithConnectTimeout(200*time.Millisecond)); err != nil {
	...
	if _, err := juzu.NewClient(addr, juzu.WithClientCert(certPem, keyPem),
		juzu.WithServerCert(fakeCertPem), juzu.WithConnectTimeout(200*time.Millisecond)); err == nil {
	...
	if _, err := juzu.NewClient(addr, juzu.WithClientCert(fakeCertPem, fakeKeyPem),
		juzu.WithServerCert(certPem), juzu.WithConnectTimeout(200*time.Millisecond)); err == nil {
		...
		if _, err = juzu.NewClient(addr, juzu.WithClientCert(fakeCertPem[:3], fakeKeyPem),
			juzu.WithConnectTimeout(200*time.Millisecond)); err == nil {
